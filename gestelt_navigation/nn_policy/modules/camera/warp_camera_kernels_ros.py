...
class DepthCameraWarpKernels:
    ...
    def draw_optimized_kernel_depth_range(
        ...
        cam_poss: wp.array(dtype=wp.vec3, ndim=2),
        cam_quats: wp.array(dtype=wp.quat, ndim=2),
        additional_rotations: wp.array(dtype=wp.quat, ndim=2),
        K_inv: wp.mat44,
        far_plane: float,
        ...
        cam_pos = cam_poss[env_id, cam_id]
        cam_quat = cam_quats[env_id, cam_id]
        fixed_additional_rotation = additional_rotations[env_id, cam_id]
        ...
        ro = cam_pos
        # tf the direction from camera to world space and normalize
        rd = wp.normalize(wp.quat_rotate(cam_quat,wp.quat_rotate(fixed_additional_rotation, uv)))
        rd_principal = wp.normalize(wp.quat_rotate(cam_quat,
            wp.quat_rotate(fixed_additional_rotation, uv_principal)
        ))  # ray direction of principal axis
        t = float(0.0)
        u = float(0.0)
        v = float(0.0)
