...
class NN_POLICY_PLANNER(object):
    ...
    #                 pva_traj_msg.type_mask = self.attitude_mode_toggle
    #                 pva_traj_msg.throttle = self.action[0,0]  #0.321
    #                 pva_traj_msg.transform.rotation.x = 0.0
    #                 pva_traj_msg.transform.rotation.y = 0.0
    #                 pva_traj_msg.transform.rotation.z = 0.707 
    #                 pva_traj_msg.transform.rotation.w = 0.707
    ...
    def upload_warp_pose_variables(self, msg):
        self.warpPoseCB(msg)

    def upload_warp_odom_variables(self, msg):
        self.warpOdomCB(msg)

    def upload_local_pose_variables(self, msg):
        self.poseCb(msg)

    def upload_local_odom_variables(self, msg):
        self.odomCb(msg)
...
if __name__=="__main__":
    ...
    full_policy_path = os.path.join(full_path, "20250326-150104/policy.pth")
    ...
    bag_path = '/home/yanrui/storage/gestelt_ws/2025-03-27-13-08-50.bag'

    # The topic you want to read from
    topic_name = '/drone0/planner_adaptor/exec_trajectory'

    # Open and read the bag
    with rosbag.Bag(bag_path, 'r') as bag:
        for topic, msg, t in bag.read_messages():
            # print(f'Time: {t.to_sec()}')
            # print(f'Message: {msg}')
            if topic == topic_name:
                print("I am here")
                nn_policy_planner.nn_evaluation()
            if topic == "/drone0/warp/local_position/pose":
                nn_policy_planner.upload_warp_pose_variables(msg)
            if topic == "/drone0/warp/local_position/odom":
                nn_policy_planner.upload_warp_odom_variables(msg)
