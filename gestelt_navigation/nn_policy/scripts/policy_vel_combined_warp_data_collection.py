...
class TEST_RENDER(object):
    ...
    def __init__(self, policy_path, pc, warp_frame):
        ...
        dims = 16 - 3
        if self.pc == True:
            self.policy = TrackCircular(input_dim=dims)
        else:
            self.policy = TrackCircular(input_dim = 10)
...
class NN_POLICY_PLANNER(object):
    ...
    def __init__(self, mission_command_mode, policy, inference_timestep, max_angular_rates, warp_jax,
                 to_transform_odom, to_transform_policy, config_param):
    ...
    def publishATT(self, type_mask, nn_action):
        ...
        ###### Data collection. I want to know the action sent, how it will affect the drone
        ...
        if self.record_now == True:
            ...
            with self.lock:
                for iii in range(1):
                    self.data_store[iii]["time_stamp"].append(now_time)
                    self.data_store[iii]["position"].append(warp_pos.squeeze(0).detach().cpu().numpy())
                    # self.data_store[iii]["velocity"].append(warp_qd[:,3:].squeeze(0).detach().cpu().numpy())
                    self.data_store[iii]["velocity"].append(self.nwu_odom[3:])
                    self.data_store[iii]["rotation"].append(warp_q.squeeze(0).detach().cpu().numpy())
                    # self.data_store[iii]["omega"].append(warp_qd[:,:3].squeeze(0).detach().cpu().numpy())
                    self.data_store[iii]["omega"].append(self.nwu_odom[:3])
                    self.data_store[iii]["action"].append(nn_action.squeeze(0).detach().cpu().numpy())
    ...
    def nn_evaluation(self, event):
        ...
        #             self.data_store[iii]["position"].append(warp_pos.squeeze(0).detach().cpu().numpy())
        #             self.data_store[iii]["velocity"].append(warp_qd[:,3:].squeeze(0).detach().cpu().numpy())
        #             self.data_store[iii]["rotation"].append(rotation_quat.squeeze(0).detach().cpu().numpy())
        #             self.data_store[iii]["omega"].append(warp_qd[:,:3].squeeze(0).detach().cpu().numpy())
        #             self.data_store[iii]["action"].append(self.action.squeeze(0).detach().cpu().numpy())
...
if __name__=="__main__":
    ...
    print("STARTING NODE")
    policy_file = "20260427-141838" #"20260407-141219" #"20260401-172740" #"20260318-154158"#"20260304-152946" #0.02 good enough for real drone 20250527-122703   0.05-to test 20250624-181715
    print(f"POLICY PATH IS {policy_file}") 

    rospack = rospkg.RosPack()
    path = rospack.get_path('nn_policy')
    full_path = os.path.join(path, "logs/circular_trajectory")
    ...
    config_params["policy_file"] = policy_file
    ...
    nn_policy_planner = NN_POLICY_PLANNER(mission_command_mode=int(mission_command_mode), policy=nn_policy,
                                          ...
                                          warp_jax=warp_jax, to_transform_odom=to_transform_odom, to_transform_policy=to_transform_policy, config_param=config_params)
