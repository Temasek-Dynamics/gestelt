...
def compute_pos_scale(cfg, device, dtype):
    ...
    if torch.any(pos_scale <= 0.0):
        raise ValueError("State position normalization scale must be positive")
    return pos_scale


def normalize_quat_xyzw_continuous(q, eps=1e-8):
    # Unit-normalize, then resolve the double-cover by flipping sign so w >= 0
    # (q and -q represent the same rotation; this picks a continuous
    # representative instead of letting the sign flip arbitrarily between steps).
    q_unit = q / q.norm(dim=1, keepdim=True).clamp_min(eps)
    sign = torch.where(q_unit[:, 3:4] < 0.0, -1.0, 1.0)
    return q_unit * sign
...
class TEST_RENDER(object):
    ...
    def __init__(self, args, policy_path, pc, warp_frame, use_gru=False, include_actions=False, include_gate_ori=False, use_rotmat_obs=False, use_so3_diff_obs=False):
        ...
        # Binary occupancy threshold (metres): a runtime debug/tuning param, not a
        # training-config value -- deliberately independent of the checkpoint's
        # args.max_range (20.0), since with depth_cam_far also at 20.0 the decoded
        # depth never exceeds 20.0, making a 20m threshold degenerate on real data.
        self.binary_threshold = float(rospy.get_param("~binary_occupancy_threshold", 10.0))
        print(f"[binary-depth] occupancy threshold = {self.binary_threshold} m "
              f"(>thresh -> 1 free, <=thresh -> 0 occupied)")
        ...
        # ---- Privileged-feature config (must match training exactly) ----
        # include_diff_pos is dropped from the core state whenever the position is
        # instead carried by the privileged block (mirrors the config's own coupling
        # between "is diff_pos privileged" and "is it in the ordinary state").
        self.priv_pos_enabled = bool(getattr(self.args, "privileged_gate_position", False))
        self.priv_enabled = bool(getattr(self.args, "privileged_gate_orientation", False))
        self.include_own_position = bool(getattr(self.args, "include_own_position_in_state", True))
        self.include_diff_pos = not self.priv_pos_enabled

        # Core state = diff_pos?(3) + own_pos?(3) + att(4) + qd(6); privileged blocks
        # (position(3), then orientation(4), matching training's append order) are
        # concatenated on top, fed as ZEROS at deploy (mirrors the confirmed
        # orientation convention -- forces reliance on vision, not on config/odometry
        # geometry, and sidesteps the unknown priv_pos_scale/priv_scale constants
        # since 0 * anything = 0). The combined aux head predicts both (dim 7 here).
        core_dim = (3 if self.include_diff_pos else 0) + (3 if self.include_own_position else 0) + 4 + 6
        self.state_dim = core_dim + (3 if self.priv_pos_enabled else 0) + (4 if self.priv_enabled else 0)
        aux_dim = (4 if self.priv_enabled else 0) + (3 if self.priv_pos_enabled else 0)
        print(f"[state] core_dim={core_dim} (diff_pos={self.include_diff_pos}, "
              f"own_pos={self.include_own_position}) + priv_pos={self.priv_pos_enabled}*3 "
              f"+ priv_ori={self.priv_enabled}*4 -> state_dim={self.state_dim}, aux_dim={aux_dim}")
        ...
        # ---- Debug: override the state with a fixed (level, zero-motion) vector ----
        ...
        # Built dynamically from this checkpoint's own layout (state_dim varies with
        # include_diff_pos/include_own_position/priv_pos_enabled/priv_enabled), so it
        # stays correctly-shaped for whichever config is loaded, rather than a literal
        # hardcoded for one specific checkpoint.
        ...
        _debug_vec = []
        if self.include_diff_pos:
            _debug_vec += [0., 0., 0.]
        if self.include_own_position:
            _debug_vec += [0., 0., 0.]
        _debug_vec += [0., 0., 0., 1.]        # att identity (level)
        _debug_vec += [0.] * 6                # qd (zero motion)
        if self.priv_pos_enabled:
            _debug_vec += [0., 0., 0.]
        if self.priv_enabled:
            _debug_vec += [0., 0., 0., 0.]
        assert len(_debug_vec) == self.state_dim, \
            f"debug vector len {len(_debug_vec)} != state_dim {self.state_dim}"
        self.debug_state_vector = torch.tensor([_debug_vec], dtype=torch.float32, device=self.device)
    ...
    def depth_to_policy_input(self,depth, max_range, invert=True):
        ...
        # Binary occupancy map from the raw metric depth: 1 = free/far (beyond
        # max_range, nothing detected), 0 = occupied/near (something within range).
        # NaN/+inf (no return) count as far (1); -inf as near (0).
        depth = torch.nan_to_num(depth.float(), nan=max_range + 1.0, posinf=max_range + 1.0, neginf=0.0)
        binary = (depth > float(max_range)).float()
        # invert=True flips the convention to near=1, far=0 (matches the old
        # continuous pipeline's "near=bright" sense); default here is the literal
        # ask: far(>max_range)=1, near(<=max_range)=0.
        return 1.0 - binary if invert else binary
    
    def normalized_state(self, q, qd, gate_centers_t, cfg, mar,
                         include_diff_pos=True, include_own_position=True):
        pos = q[:, :3]
        att = normalize_quat_xyzw_continuous(q[:, 3:])
        pos_scale = compute_pos_scale(cfg, q.device, q.dtype)
        vel_scale = float(getattr(cfg, "state_vel_scale", 4.5))
        omega_scale = float(getattr(cfg, "state_angvel_scale", mar))
        if vel_scale <= 0.0 or omega_scale <= 0.0:
            raise ValueError("State normalization scales must be positive")
        qd_n = torch.cat([qd[:, :3] / omega_scale, qd[:, 3:] / vel_scale], dim=1)
        parts = []
        if include_diff_pos:
            parts.append((gate_centers_t - pos) / pos_scale)
        if include_own_position:
            parts.append(pos / pos_scale)
        parts.append(att)
        parts.append(qd_n)
    ...
    def evaluate_(self, pos, att, qd, depth_img):
        ...
        # Binary occupancy map: >binary_threshold -> 1 (far/free), <=binary_threshold
        # -> 0 (near/occupied). Uses a dedicated threshold, NOT the checkpoint's
        # args.max_range (20.0) -- with depth_cam_far also defaulting to 20.0, the
        # decoded depth is capped exactly at max_range, so nothing could ever exceed
        # it and the "free" class was unreachable. invert is fixed to False (not
        # read from invert_depth, which was written for the old continuous
        # near=bright encoding and would flip this to the opposite convention).
        depth = self.depth_to_policy_input(
            depth_img,
            max_range=self.binary_threshold,
            invert=False,
        )
        ...
        if depth.shape[-2:] != (self.img_h, self.img_w):
            ...
            # Bilinear resize blends the 0/1 mask at edges (anti-aliased gray),
            # which isn't meaningful for a binary occupancy map -- re-threshold at
            # 0.5 to restore a strictly {0, 1} image after downsampling.
        ...
        state_core = self.normalized_state(q, qd, gate_centers_t, self.args, self.mar,
                                           include_diff_pos=self.include_diff_pos,
                                           include_own_position=self.include_own_position)
        # Privileged blocks, in training's append order (position, then orientation).
        # Fed as ZEROS at deploy -- mirrors the confirmed orientation convention (forces
        # reliance on vision rather than config/odometry geometry) and sidesteps the
        # unknown priv_pos_scale/priv_scale constants (0 * anything = 0 regardless).
        blocks = [state_core]
        if self.priv_pos_enabled:
            blocks.append(torch.zeros(state_core.shape[0], 3, dtype=state_core.dtype, device=state_core.device))
        if self.priv_enabled:
            blocks.append(torch.zeros(state_core.shape[0], 4, dtype=state_core.dtype, device=state_core.device))
        state = torch.cat(blocks, dim=1).to(self.device)
        # print(state)
        if self.debug_override_state:
...
class NN_POLICY_PLANNER(object):
    ...
    def randomize_init_pos(self):
        ...
        x = random.uniform(wx - 7.0, wx - 6.0)
