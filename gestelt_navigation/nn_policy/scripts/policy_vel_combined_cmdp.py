...
class NN_POLICY_PLANNER(object):
    ...
    def __init__(self, mission_command_mode, policy, inference_timestep, max_angular_rates, warp_jax,
        ...
        init_pos = self.policy.init_pos.detach().cpu().numpy()
        self.init_pos_numpy = init_pos
        ## Target location (sampled goal), used for the episode-end proximity check
        self.target_pos_numpy = self.policy.t_pos.detach().cpu().numpy()
    ...
    def publish_start_target_markers(self):
        ...
        now = rospy.Time.now()
        init_pos = np.asarray(self.init_pos_numpy).reshape(-1)
        target_pos = np.asarray(self.target_pos_numpy).reshape(-1)
    ...
    def publish_trajectory_point(self):
        ...
        pose.pose.position.x = float(self.drone_pos[0])
        pose.pose.position.y = float(self.drone_pos[1])
        pose.pose.position.z = float(self.drone_pos[2])
