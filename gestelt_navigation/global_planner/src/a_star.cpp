    ...
    const bool& makeGoalBubble)
{
    ...
    while(!q.empty()) {
        ...
        for (int dx=-1; dx<=1; dx++) {
            int nx = cur_cell.x + dx;
            if (nx<0 || nx >= dyn_voro_arr_[origin_cell.z_cm]->getSizeX()) {
                continue; // Skip if outside map
            }
            for (int dy=-1; dy<=1; dy++) {
                int ny = cur_cell.y + dy;
                ...
                if (ny<0 || ny >= dyn_voro_arr_[origin_cell.z_cm]->getSizeY()){
                    continue; // Skip if outside map
                } 
...
{
    std::cout << "generatePlanVoroT" << std::endl;
                                        ...
                                        std::function<double(const VCell_T&, const VCell_T&)> cost_function)
{
    ...
    // // Create voronoi bubble around start and goal
    // expandVoronoiBubbleT(start_node,    false );
    // expandVoronoiBubbleT(goal_node,     true );
    ...
    {
        // std::cout << "[space-time A*] Iteration " << num_iter << std::endl;
        // if (num_iter%100 == 1){
        //     std::cout << "[a_star] Iteration " << num_iter << std::endl;
        //     publishClosedList(getClosedListVoronoi(), closed_list_viz_pub_, "local_map_origin");
        //     // ros::Duration(0.1).sleep();
        ...
        {   
            ...
            DblPoint nb_2d_pos;
            dyn_voro_arr_[cur_node.z_cm]->idxToPos(IntPoint(nb_node_eig(0), nb_node_eig(1)), nb_2d_pos);
