...
class LineSegment : public DecompBase<Dim> {
  ...
  protected:
    ...
    void add_local_bbox(Polyhedron<Dim> &Vs) {
      ...
      Vs.add(Decomp::Hyperplane<Dim>(pp1, dir_h));
      Vs.add(Decomp::Hyperplane<Dim>(pp2, -dir_h));

      // along y
      Vecf<Dim> pp3 = p2_ + dir * this->local_bbox_(0);
      Vecf<Dim> pp4 = p1_ - dir * this->local_bbox_(0);
      Vs.add(Decomp::Hyperplane<Dim>(pp3, dir));
      Vs.add(Decomp::Hyperplane<Dim>(pp4, -dir));
      ...
      if(Dim > 2) {
        ...
        Vs.add(Decomp::Hyperplane<Dim>(pp5, dir_v));
        Vs.add(Decomp::Hyperplane<Dim>(pp6, -dir_v));
      }
    }
