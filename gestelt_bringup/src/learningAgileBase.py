...
class LearningAgileBase:
    ...
    def get_reg_det(self):
        ...
        self.reg_det=self.mission_cfg['penalty']['det_reg_w'] * (np.log(1 + np.exp(-np.linalg.det(m))))#+(np.linalg.det(m)-1)**2)
       
        adj_m_T = np.linalg.det(m)*np.linalg.inv(m).T
        dreg_det_dm = self.mission_cfg['penalty']['det_reg_w']*((-np.exp(-np.linalg.det(m)))/ (1 + np.exp(-np.linalg.det(m))) * adj_m_T) #+ 2*(np.linalg.det(m)-1) * adj_m_T)
        
        self.regularize += self.reg_det
        self.dreg_dz[:,3:12]+=dreg_det_dm.flatten()

    def get_reg_orthogonal(self):
        """
        regularize the output of the neural network m, the m should be orthogonal
        """
        m=self.np_nn_out[3:12].reshape(3,3)
        self.reg_orthogonal = self.mission_cfg['penalty']['orthogonal_reg_w']*np.trace(np.dot(m,m.T)-np.eye(3))
        dreg_orth_dm = self.mission_cfg['penalty']['orthogonal_reg_w']*2*m
        
        self.regularize += self.reg_orthogonal
        self.dreg_dz[:,3:12]+=dreg_orth_dm.flatten()
    ...
    def get_euler_nn(self):
        ...
        """
        euler_nn=recover_euler_from_9d(self.np_nn_out,deg_unit=True)
        return euler_nn
