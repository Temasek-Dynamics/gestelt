...
def check_traj_server_states(des_traj_server_state):
    ...
    for server_state in server_states.items():
        print(f"Current state: {server_state[1].traj_server_state}")
...
def main():
    ...
    print("Setting to ACTUATOR_TEST mode!")
    # Go from IDLE to ACTUATOR_TEST
    while not rospy.is_shutdown():
        get_server_state_callback()
        if check_traj_server_states("ACTUATOR_TEST"):
            break
        publish_server_cmd(5)   # Send actuator test event
        print("tick!")
        rate.sleep()
