...
def plot_pose(
    ...
    times = times - t_init
    plt.plot(times[mask], positions[mask, 0], label='x')
    plt.plot(times[mask], positions[mask, 1], label='y')
    plt.plot(times[mask], positions[mask, 2], label='z')
    ...
    plt.plot(times_NN[mask_NN], NN_positions[mask_NN, 0], '--', color='C0', label='NN x')
    plt.plot(times_NN[mask_NN], NN_positions[mask_NN, 1], '--', color='C1', label='NN y')
    plt.plot(times_NN[mask_NN], NN_positions[mask_NN, 2], '--', color='C2', label='NN z')
    ...
    # Orientation plot
    plt.figure(figsize=(4, 3))
    plt.plot(times[mask], orientations[mask, 0], label='roll')
    plt.plot(times[mask], orientations[mask, 1], label='pitch')
    plt.plot(times[mask], orientations[mask, 2], label='yaw')
    
    # Plot the NN orientations as dashes
    plt.plot(times_NN[mask_NN], NN_orientations[mask_NN, 0], '--', color='C0',  label='NN roll')
    plt.plot(times_NN[mask_NN], NN_orientations[mask_NN, 1], '--', color='C1',  label='NN pitch')
    plt.plot(times_NN[mask_NN], NN_orientations[mask_NN, 2], '--', color='C2',  label='NN yaw')
...
def plot_vel(times,t_init, velocities):
    """ Plot velocity data."""
    plt.figure(figsize=(4, 3))
    mask_start = times[:] > t_init
    mask_end = times[:] < t_init + 2.5
    ...
    # shift times to start from t_init
    times = times - t_init
    plt.plot(times[mask], velocities[mask, 0], label='x_axis')
    plt.plot(times[mask], velocities[mask, 1], label='y_axis')
    plt.plot(times[mask], velocities[mask, 2], label='z_axis')
...
def plot_NN_weights(times, t_init, weights):   
    ...
    # shift times to start from t_init
    times = times - t_init
    plt.plot(times[mask], weights[mask, 0], label='x_axis')
    plt.plot(times[mask], weights[mask, 1], label='y_axis')
    plt.plot(times[mask], weights[mask, 2], label='z_axis')
    ...
    plt.figure(figsize=(4, 3))
    plt.plot(times[mask], weights[mask, 3], label='x_axis')
    plt.plot(times[mask], weights[mask, 4], label='y_axis')
    plt.plot(times[mask], weights[mask, 5], label='z_axis')
    ...
    plt.figure(figsize=(4, 3))
    plt.plot(times[mask], weights[mask, 6], label='x_axis')
    plt.plot(times[mask], weights[mask, 7], label='y_axis')    
...
if __name__ == "__main__":
    ...
    # Plot
    plot_pose(times_pose,times_NN_pose, t_init, positions, orientations, NN_positions, NN_orientations)
    plot_vel(times_vel, t_init, velocities)
    plot_NN_weights(times_weights, t_init, weights)
