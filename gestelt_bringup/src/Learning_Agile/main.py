...
def receive_states(start,end,gate):
    inputs[0:3]=start
    inputs[3:6]=end
    gate_point0=gate

    return inputs,gate_point0

def gate_initialization (gate_point0):
    ...
    return gate1,g_init_p,gate_point,gate_move, V ,w
...
inputs,gate_point0 = receive_states(start=np.array([0,1.8,1.4]),end=np.array([0,-1.8,1.4]),gate=gate_point0)
gate1,g_init_p,gate_point,gate_move, V ,w = gate_initialization(gate_point0)
