...
def DifferentiableCollisionsWrapper(line_centers,
    ...
    for i in range(4): # P_obs[1],P_obs[3] (left and right walls)
        ...
        reward+=-(1000*(alpha_i-1.1)**2)
        dalpha_dstate_drone[0:3] += -2000*(alpha_i-1.1) * dalpha_i_dstate_np[0:3]
        dalpha_dstate_drone[6:10] += -2000*(alpha_i-1.1) * dalpha_i_dstate_np[3:7]

    # keep at the center reward
    # reward_center = - 0.1 * (drone_state[0]-0)**2
    # reward_center += - 0.1 * (drone_state[2]-0)**2
    # reward+=reward_center

    # dalpha_dstate_drone[0] -= 0.1 * 2 * (drone_state[0]-0)
    # dalpha_dstate_drone[2] -= 0.1 * 2 * (drone_state[2]-0) 
