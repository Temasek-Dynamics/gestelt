...
def main():
    ...
    for i in range(num_passes):
        ...
        radius = radius - radius*(i/num_passes)
        height = height + (2 - 1.4)*i/num_passes
        height = min(2.2, height)
        radius - min(1.8, radius)
        waypoints.append(create_pose(radius,0.0,height))   
        waypoints.append(create_pose(0.0,-radius ,height)) 
        waypoints.append(create_pose(-radius, 0.0, height))
        waypoints.append(create_pose(0.0,radius,height))
